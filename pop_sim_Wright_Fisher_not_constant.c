...
void freq_func(int x, typ* array, int size){
  ...
  for(int i=0; i<size; i++){
	...
	fprintf(f2, "%llu:%Lf\t",array[i],(long double)freq[i]/(long double)size);
	fclose(f2);
      }
...
void mutation_process(int number_of_mutations, long long int positions, typ* pop, int generation){
  ...
  typ* position_of_mutations_array=(typ*)calloc(number_of_mutations,sizeof(typ));
    ...
    freq_func(2,position_of_mutations_array,number_of_mutations);
...
int main(int argc, char* argv[]){
  ...
  for (int i=0;i<generations;i++){
    ...
    freq_func(1,pop_prev,size);// τρέχει σωστά και βγάζει συχνότητα γοιδιωμάτων μετα την μετάλλαξη.                                                                                                           
