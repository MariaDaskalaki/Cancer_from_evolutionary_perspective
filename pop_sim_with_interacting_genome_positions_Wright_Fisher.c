...
void setbits_positions(int N, typ* pop){
  ...
  for (int j=0;j<64;j++){
    ...
    fprintf(f2,"%d:%f\t",j,(double)position_array[j]/((double)N*intsize));
    fclose(f2);
  }
  free(position_array);
}
...
float* sum_array(float* array, int size){
  ...
  float after_sum=0;
  float* sum_array=(float*)calloc(size,sizeof(int));

  for (int i=0; i<size; i++){
    sum_array[i]=previous_sum;
    previous_sum+=array[i];
  }

  for (int i=size-1; i>=0; i--){
    sum_array[i]+=after_sum;
    after_sum+=array[i];
  }

  for (int i=0; i<size; i++){
    sum_array[i]=2*array[i]+sum_array[i];
    //fprintf(stderr, "%f\n", sum_array[i]);
  }
...
float weight_selection_position1(float* array,unsigned k){
  ...
  //weight_position_array[i]=(float)rand()/(float)RAND_MAX*10-1;
  ...
  for (int i=0;i<intsize;i++){
    com_weight_position_array[0]=new_weight_position_array[0];
    if (i>0){
      com_weight_position_array[i]=new_weight_position_array[i]+new_weight_position_array[i-1];
    }
  }
...
void com_fitness_func2(float* array,int N, double* fitness, double* com_fitness, typ *pop, int selected_pos){
  // int k=rand()%64 +1;                                                                                                                                                                                    
  ...
  for (int i=0;i<N;i++){
    ...
    if ((typ)(pop[i] & fitfactor) == (typ)0){
      ...
      fitness[i]= weight_selection_position1(array,i); // υποτίθεται είναι με τις αλληλεπιδράσεις (πολυμεταβλητό γραμμικό μοντέλο), ενώ το weight_selection_position είναι με το απλό γραμμικό μοντέλο όπου κάθε θέση είναι ανεξάρτητη.
      //fitness[i]=weight_selection_position1(array,selected_pos-1);
    }
...
void selection_func(float* array,int N1, int N2,typ* pop1, typ* pop2, double *fitness, double* com_fitness,int selected_pos){
  ...
  com_fitness_func2(array,N1,fitness,com_fitness,pop1,selected_pos);
...
int main(int argc, char* argv[]){
  ...
  for (int i=0;i<generations;i++){
    ...
    selection_func(weight_position_array,size,next_size, pop_prev, pop_cur, fitness, com_fitness,selected_pos);
    ...
    setbits_positions(size,pop_prev);
