...
float weight_selection_position1(float* array,gsl_rng* r,unsigned k){
  ...
  //if (i>0){
  //  com_weight_position_array[i]=new_weight_position_array[i]+com_weight_position_array[i-1];
  //}
  //}
  ...
  for (int i=0; i<intsize; i++){
    norm_new_weight_position_array[i]=new_weight_position_array[i]-min_fitness;
    // fprintf(stderr,"%f\n",norm_new_weight_position_array[i]);
  }


  float x=norm_new_weight_position_array[k];
...
void com_fitness_func2(float* array,gsl_rng* r,int N, double* fitness, double* com_fitness, typ *pop, int selected_pos){
  ...
  for (int i=0;i<N;i++){
    ...
    if ((typ)(pop[i] & fitfactor) == (typ)0){
      ...
      fitness[i]= weight_selection_position1(array,r,i); // υποτίθεται είναι με τις αλληλεπιδράσεις (πολυμεταβλητό γραμμικό μοντέλο), ενώ το weight_selection_position είναι με το απλό γραμμικό μοντέλο όπου κάθε θέση είναι ανεξάρτητη.
...
float* weight_position_array_func(gsl_rng*r,int size){
  ...
  float* weight_position_array=(float*)calloc(size,sizeof(float));

  for (int i=0;i<size;i++){
    weight_position_array[i]=gsl_ran_gaussian(r,1);
    // weight_position_array[i]=(float)rand()/(float)RAND_MAX*10-11;                                                                                                                                        
  }
  return weight_position_array;
  free(weight_position_array);
}
