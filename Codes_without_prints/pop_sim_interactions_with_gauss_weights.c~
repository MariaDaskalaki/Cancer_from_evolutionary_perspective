...
int Gompertz_discrete_model(int initial, int t, long double growth_rate, long double decay_rate, int prev_pop_size){
  long double c;
  c=(long double)growth_rate/(long double)decay_rate;
  int newSize=prev_pop_size+(growth_rate*initial*1)*(exp(c*(1-(exp(-decay_rate*t)))));
...
int * population_change(int initialN, int generations,long double growth_rate,gsl_rng* r) // growth_rate=ln(2)/Tc, where Tc=doubling time of cells and here it is considered to be 1 generation.
{
    ...
    for (int i = 1; i < generations; ++i)
    {
      ...
      //population_array[i]=Gompertz_discrete_model(initialN,i,growth_rate,gsl_ran_lognormal(r,-2.9,0.71)); // check paper Estimating tumor growth rates in vivo, for parameters.
      population_array[i]=Gompertz_discrete_model(initialN,i,growth_rate,0.2,population_array[i-1]);
