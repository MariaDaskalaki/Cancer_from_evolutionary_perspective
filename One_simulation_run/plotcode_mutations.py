...
############# Code for plotting the mutations frequencies evolution in time ##################
...
with open("Frequencies_mutations_CA.txt",'r') as f:
    data=f.readlines()
    ...
    last_muts=[]
    for k in mut_dict_final.keys():
        if k==gen_list[-1]:
            #print (mut_dict_final[k])
            for j in mut_dict_final[k].keys():
                if mut_dict_final[k][j]!=0:
                    last_muts.append(j)
   # last_muts=[j for j in mut_dict_final[k].keys() if mut_dict_final[k][j]!=0 for k,v in mut_dict_final.items() if k==gen_list[-1]]
    print(last_muts)
    ...
    freq_thres=(max(float_freq))*0.95
    #freq_thres=max(float_freq)
    #print (freq_thres)
    ...
    df_new_last=df.loc[:,('{}'.format(i) for i in last_muts)]
    df_new_last.plot(figsize=(10,6), xticks=range(0, len(gen_list),1000)).legend(title='', bbox_to_anchor=(1, 1))
    #df_new.plot(figsize=(6000,6), xticks=range(0,len(gen_list),1000)).legend(title='', bbox_to_anchor=(1, 1))
    plt.title("Fixed mutations evolution")
    plt.xlabel("Generations")
    plt.ylabel("Frequencies")
    plt.show()
