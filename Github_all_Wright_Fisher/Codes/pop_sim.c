...
char * bitfunc(typ n){
  ...
  //assert(n > 0);
  //unsigned int u=*(unsigned int*)&n; // store integer n in a positive(as we have unsigned type) pointer variable of integer u. & is a misc operator in c and &a returns the actual address of the variable a. 
  int i;
  typ mask = (typ)1 << (nbits-1); // << left shift operator. The left shift and right shift operators should not be used for negative numbers. The result of is undefined behaviour if any of the operands is a negative number. Thats why we use unsigned type. If the number is shifted more than the size of integer the behaviour is undefined. For example 1 << 33 is undefined if integers are stored using 32 bits. Here we have stored integers of sixe nbits=intsize=define 32. >> Binary Left Shift Operator. The left operands value (here is 1) is moved left by the number of bits specified by the right operand ( here is 32). So here its like filling the 32 positions from right to left. 
  for (i=0;i<nbits;i++,mask>>=1){ // mask>>=1 is the same as mask=mask>>1 so the left from above 1 is moved right by 1 bit.
    s[i]=((n&mask)!=0)+'0'; // & Binary AND Operator copies a bit to the result if exists in both operands. Here we check if 1 (which is !=0) is in both u and mask we place and what is left is filled up with 0.
  }
  // printf("%llu, %llu, %s\n", n, mask, s);
...
int main(int argc, char* argv[]){
  ...
  double mu=0.01;
  //long long int total_pop_pos=N*intsize; νομίζω δεν είναι σωστό εδώ γτ κάθε φορά το mutation θα γίνεται μόνο ανάμεσα στα άτομα του αρχικού πληθυσμού. 
  ...
  for (int i=0;i<generations;i++){
    ...
    for(int k = 0; k < size; ++k){
      char* s=bitfunc(pop_prev[k]); 
      fprintf(stderr,"%d  -- The genome (WITHOUT) of individual %d of the current population is %s\n", i, k,s);
      free(s);
    }
    

    int* position_of_mutations_array=(int*)calloc(numberOfMutations,sizeof(int));
    for (int j=0;j<numberOfMutations;j++){
      position_of_mutations_array[j]=mutate(total_pop_pos, pop_prev, i); // οι θέσεις που έγιναν μεταλλάξεις μπαινουν σε ενα array
    ...
    for(int k = 0; k < size; ++k){
      char* s=bitfunc(pop_prev[k]);
      int num_of_setbits=countSetBits(pop_prev[k]);
  ...
  for (int i=0;i<size;i++){
    int setBits = countSetBits(pop_cur[i]);
    int commonBits = countSetBits(pop_cur[i] & global_fitfactor); // οι κοινες θεσεις ( άσσοι) μεταξύ των άσσεων του fitfactor και του μεταλλαγμένου γονιδιώματος).
